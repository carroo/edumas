                                ...
                                <div>
                                    <div class="row d-flex justify-content-between mb-4">
                                        ...
                                        <form action="{{ route('pengaduans.destroy', $item->id) }}" method="post"
                                            ...
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="badge badge-danger rounded-lg py-2 px-5"
                                                style="height: fit-content;">Hapus</button>
                                        </form>
                                    </div>
                                    <h6>{{ $item->deskripsi }}</h6>
                                </div>
                                <div>
                                    <p>{{ $item->lokasi }}, {{ date('M d, Y', strtotime($item->tgl)) }}</p>
                                    <div class="d-flex justify-content-between">
                                        <div>
                                            <span class="badge badge-danger rounded-pill px-2 py-1">Penting dan
                                                Mendesak</span>
                                            <span
                                                class="badge badge-success rounded-pill px-2 py-1">{{ $item->kategori->name }}</span>
                                        </div>
                                        <span
